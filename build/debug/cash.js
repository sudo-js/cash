(function(window) {
  ...
  function addPx(v, k) {
    return (typeof v === 'number' && (!k || !cssNum[k])) ? v + 'px' : v;
  }
  ...
  cash.removeAttr = function(key) {
    var rem = isArray(key) ? function(el) {
  ...
  cash.addClass = function(cls) {
    ...
    this.q.forEach(function(el) {
      if (el.classList) ary.forEach(function(n) {
        el.classList.add(n);
      });
    });
    return this;
  ...
  cash.css = function(key, val) {
    var ary = isString(key) ? undefined : keys(key),
      set = ary ? function(el) {
        ary.forEach(function(i) {
          el.style[i] = addPx(key[i], i);
        });
      } :
      function(el) {
        el.style[key] = addPx(val, key);
      };
    this.q.forEach(set);
    return this;
  };
  ...
  cash.height = function(val) {
    return this._hw_('height', val);
  };
  // ###_hw_
  ...
  cash._hw_ = function(key, val) {
    var obj = {
      ...
      node = this.q[0],
      d = isDocument(node) ? node.documentElement : null,
      w = isWindow(node) ? node : null,
      type = obj[key],
      o;
  ...
  // `param` {number|string} `val`. Optional value to be set
  // `returns` {number|object} The height if a getter, cash if a setter
  cash.width = function(val) {
    return this._hw_('width', val);
  };
  // #is
  ...
  cash.is = function(sel) {
    this.q = this.q.filter(function(el) {
